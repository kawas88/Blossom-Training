...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  ...
  const next = pickNextExerciseId(exercises, session.current_exercise_id)
  if (next === null) {
    // No further trainer-paced exercises — wrap automatically.
    ...
    return NextResponse.json({ session: data, wrapped: true })
  }

  const { data, error } = await supabase
    .from('training_sessions')
    .update({
      current_exercise_id: next,
      updated_at: new Date().toISOString(),
    })
    .eq('id', session.id)
    .select('*')
